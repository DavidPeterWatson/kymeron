...
class ToolProbe:
    def __init__(self, config):
        ...
        self.multi_axis_probe_session = MultiAxisProbeSessionHelper(config,
                                                      ProbeEndstopWrapper(
                                                          config, 'x'),
                                                      ProbeEndstopWrapper(
                                                          config, 'y'),
        ...
        self.lift_speed = config.getfloat('lift_speed',
                                          self.multi_axis_probe_session.lift_speed)
    ...
    def probe_xy(self, toolhead, top_pos, direction, gcmd, samples=None):
        ...
        toolhead.manual_move([None, None, top_pos[2] - self.lower_z],
                             self.lift_speed)
        return self.multi_axis_probe_session.run_probe(gcmd, direction, samples=samples,
    ...
    def locate_sensor(self, gcmd):
        ...
        downPos = self.multi_axis_probe_session.run_probe(gcmd, "z-", samples=1)
        ...
        center_z = self.multi_axis_probe_session.run_probe(gcmd, "z-", speed_ratio=0.5)[
            2]
    ...
    def cmd_TOOL_CALIBRATE_PROBE_OFFSET(self, gcmd):
        ...
        nozzle_z = self.multi_axis_probe_session.run_probe(gcmd, "z-", speed_ratio=0.5)[2]
    ...
    def cmd_TOOL_CALIBRATE_QUERY_PROBE(self, gcmd):
        ...
        endstop_states = [probe.query_endstop(print_time) for probe in self.multi_axis_probe_session.mcu_probe] # Check the state of each axis probe (x, y, z)
...
class MultiAxisProbeSessionHelper:
    ...
    def _rocking_probe(self, speed, axis, sense, max_distance):
        toolhead = self.printer.lookup_object('toolhead')
    ...
    def run_probe(self, gcmd, direction='z-', speed_ratio=1.0, samples=None,
                  max_distance=100.0):
        ...
        while len(positions) < sample_count:
            # Probe position
            pos = self._rocking_probe(speed, axis, sense, max_distance)
...
class MultiAxisProbe:
    def __init__(self, config):
        self.printer = config.get_printer()
        self.tool_probe = ToolProbe(config)
        self.mcu_probe = self.tool_probe.mcu_probe[2]
